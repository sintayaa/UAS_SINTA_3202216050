...
$semuaartikel_sort_id = $conn->query("SELECT artikel.*, kategori.nama AS kategori_nama FROM artikel 
                        ...
                        WHERE kategori.nama = 'Technology'
    ...
    <div class="container py-lg-4 py-md-3">
        ...
        <div class="row">
            <div class="col-lg-8 most-recent">
                ...
                <div class="row">
                    <?php
                    $isFirst = true; // Set a flag for the first item
                    while ($row = $semuaartikel_sort_id->fetch_assoc()):
                    ?>
                        <div class="col-lg-<?= $isFirst ? '12' : '6' ?> col-md-6 item mb-4 pt-lg-3 <?= $isFirst ? 'first-article' : '' ?>">
                            <div class="card">
                                <div class="card-header p-0 position-relative">
                                    <a href="detail.php?id=<?= $row['id'] ?>">
                                        <img class="card-img-bottom d-block radius-image <?= $isFirst ? 'larger-image' : '' ?>" src="admin/uploads/<?= $row['foto'] ?>" alt="Card image cap">
                                    </a>
                                </div>
                                <div class="card-body p-0 blog-details">
                                    <a href="detail.php?id=<?= $row['id'] ?>" class="blog-desc <?= $isFirst ? 'larger-title' : '' ?>"><?= $row['judul'] ?></a>
                                    <p><?= substr($row['deskripsi'], 0, 100) ?>...</p>
                                    <div class="author align-items-center mt-3 mb-1">
                                        <a href="#author"><?= $row['penulis'] ?></a> in <a href="#url"><?= $row['kategori_nama'] ?></a>
                                    </div>
                                    <ul class="blog-meta">
